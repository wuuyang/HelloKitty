package com.example.carousel.controls;
...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset){
    	...
    	float x = - (float)(diameter/2  * sin(angleOffset)) + diameter/2 - child.getWidth()/2;
    	float z = diameter/2 * (1.0f - (float)cos(angleOffset));
    	float y = - getHeight()/2 + (float) (z * sin(mTheta));
